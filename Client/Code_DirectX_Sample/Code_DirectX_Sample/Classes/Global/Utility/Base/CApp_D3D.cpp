...
CApp_D3D::CApp_D3D(HINSTANCE a_hInst,
	int a_nOptShow, const SIZE& a_rstSizeWnd) : CApp_Wnd(a_hInst, a_nOptShow, a_rstSizeWnd)
{
...
void CApp_D3D::Init(void)
{
	...
	m_pStateDS = this->CreateStateDS();
	m_pStateRasterizer = this->CreateStateRasterizer();
...
void CApp_D3D::Release(bool a_bIsDestroy)
{
	...
	if(a_bIsDestroy)
	{
		...
		SAFE_RELEASE(m_pStateDS);
		SAFE_RELEASE(m_pStateRasterizer);
...
void CApp_D3D::OnUpdate(const float a_fTimeDelta)
{
	char szStrDebugInfo[MAX_PATH] = "";

	sprintf(szStrDebugInfo, "Delta Time: %f sec\nRunning Time: %f sec",
		a_fTimeDelta, CManager_Time::GetInst()->GetTimeRunning());

	m_oStrDebugInfo = szStrDebugInfo;
}

void CApp_D3D::OnRender(HDC a_hDC)
{
	m_pDevice->RSSetState(m_pStateRasterizer);
	m_pDevice->OMSetDepthStencilState(m_pStateDS, 0);

	m_pDevice->OMSetRenderTargets(1, &m_pViewRT, m_pViewDS);

	m_pDevice->ClearRenderTargetView(m_pViewRT, m_stColorClear);
	m_pDevice->ClearDepthStencilView(m_pViewDS, D3D10_CLEAR_DEPTH | D3D10_CLEAR_STENCIL, 1.0f, 0);

	this->OnRender(m_pDevice);
}

void CApp_D3D::OnLateRender(HDC a_hDC)
{
	this->OnLateRender(m_pDevice);
	...
	this->OnLateRender(m_pXSprite);
	m_pXSprite->End();
}

std::string CApp_D3D::GetPathEffect(void)
{
	return "";
}
...
{
	m_pViewRT = this->CreateViewRT();
	m_pViewDS = this->CreateViewDS();
...
{
	D3DXVECTOR3 stPos(0.0f, 0.0f, m_pCamera->GetDistance() * -1.0f);
	D3DXVECTOR3 stPosTarget(0.0f, 0.0f, 0.0f);
	D3DXVECTOR3 stDirectionUp(0.0f, 1.0f, 0.0f);

	D3DXMATRIXA16 stMatrixView;
	D3DXMATRIXA16 stMatrixProjection;

	D3DXMatrixLookAtLH(&stMatrixView, &stPos, &stPosTarget, &stDirectionUp);

	D3DXMatrixPerspectiveFovLH(&stMatrixProjection,
		m_pCamera->GetFOV(), this->GetSizeWnd().cx / (float)this->GetSizeWnd().cy, 0.1f, 25000.0f);

	m_pCamera->SetMatrixView(stMatrixView);
	m_pCamera->SetMatrixProjection(stMatrixProjection);
}
...
void CApp_D3D::SetupViewport(void)
{
	...
	stViewport.Width = this->GetSizeWnd().cx;
	stViewport.Height = this->GetSizeWnd().cy;
...
void CApp_D3D::HandleMsgSize(HWND a_hWnd, WPARAM a_wParams, LPARAM a_lParams)
{
	CApp_Wnd::HandleMsgSize(a_hWnd, a_wParams, a_lParams);
	this->Release();
	...
	m_pSwapChain->ResizeBuffers(0,
		this->GetSizeWnd().cx, this->GetSizeWnd().cy, stDescSwapChain.BufferDesc.Format, stDescSwapChain.Flags);
...
IDXGISwapChain* CApp_D3D::CreateSwapChain(void)
{
	...
	stDescSwapChain.OutputWindow = this->GetHandleWnd();
	...
	stDescSwapChain.BufferDesc.Width = this->GetSizeWnd().cx;
	stDescSwapChain.BufferDesc.Height = this->GetSizeWnd().cy;
	...
	stDescSwapChain.SampleDesc.Quality = Func::GetLevelsMultiSampleQuality(stDescSwapChain.BufferDesc.Format,
		stDescSwapChain.SampleDesc.Count) - 1;
...
ID3D10RenderTargetView* CApp_D3D::CreateViewRT(void)
{
	ID3D10Texture2D* pBufferBack = nullptr;
	m_pSwapChain->GetBuffer(0, __uuidof(ID3D10Texture2D), (LPVOID*)&pBufferBack);

	ID3D10RenderTargetView* pViewRT = nullptr;
	m_pDevice->CreateRenderTargetView(pBufferBack, nullptr, &pViewRT);

	SAFE_RELEASE(pBufferBack);
	return pViewRT;
}

ID3D10DepthStencilView* CApp_D3D::CreateViewDS(void)
{
	auto pBufferDS = Factory::CreateTexture2D(this->GetSizeWnd().cx,
		this->GetSizeWnd().cy, DXGI_FORMAT_D24_UNORM_S8_UINT, D3D10_BIND_DEPTH_STENCIL, FLAGS_CPU_ACCESS_NONE);
	...
	ID3D10DepthStencilView* pViewDS = nullptr;
	m_pDevice->CreateDepthStencilView(pBufferDS, &stDescDSView, &pViewDS);

	SAFE_RELEASE(pBufferDS);
	return pViewDS;
}

ID3D10DepthStencilState* CApp_D3D::CreateStateDS(void)
{
	D3D10_DEPTH_STENCIL_DESC stDescDS;
	ZeroMemory(&stDescDS, sizeof(stDescDS));

	ID3D10DepthStencilState* pStateDS = nullptr;
	m_pDevice->CreateDepthStencilState(&stDescDS, &pStateDS);

	return pStateDS;
}

ID3D10RasterizerState* CApp_D3D::CreateStateRasterizer(void)
{
	...
	ID3D10RasterizerState* pStateRasterizer = nullptr;
	m_pDevice->CreateRasterizerState(&stDescRasterizer, &pStateRasterizer);

	return pStateRasterizer;
}
...
LPDIRECT3DDEVICE9 CApp_D3D::CreateDevice9(void)
{
	...
	stParams.hDeviceWindow = this->GetHandleWnd();
	...
	stParams.BackBufferWidth = this->GetSizeWnd().cx;
	stParams.BackBufferHeight = this->GetSizeWnd().cy;
	stParams.BackBufferFormat = D3DFMT_A8R8G8B8;

	stParams.MultiSampleType = D3DMULTISAMPLE_NONE;

	stParams.MultiSampleQuality = Func::GetLevelsMultiSampleQuality(stParams.BackBufferFormat,
	...
	m_pD3D9->CreateDevice(stCapsDevice.AdapterOrdinal,
		stCapsDevice.DeviceType, this->GetHandleWnd(), nFlagsCreate | D3DCREATE_MULTITHREADED, &stParams, &pDevice);
