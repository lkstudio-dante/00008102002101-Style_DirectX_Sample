...
CManager_Snd::CManager_Snd(void)
	:
	m_pBGM(CFactory::CreateObj<CSnd>())
{
	// Do Something
}
...
void CManager_Snd::Release(void)
{
	SAFE_DELETE(m_pBGM);
	...
	this->EnumerateSFXs([&](auto a_pSnd) -> void
	{
		SAFE_DELETE(a_pSnd);
	});
}
...
CSnd* CManager_Snd::FindSFX_Playable(const std::string& a_rPath_Snd)
{
	...
	if(m_oMapSFXs.find(a_rPath_Snd) == m_oMapSFXs.end())
	{
		...
		m_oMapSFXs.insert(std::make_pair(a_rPath_Snd, oVectorSFXs));
	}

	// 사운드 생성이 가능 할 경우
	if(m_oMapSFXs[a_rPath_Snd].size() < ::G_NUM_SFXS_MAX_DUPLICATE)
	{
...
LPDIRECTSOUNDBUFFER8 CManager_Snd::CreateBuffer_PrimarySnd(void)
{
	...
	return CFactory::CreateBuffer_Snd(0, stInfo_Wave, DSBCAPS_PRIMARYBUFFER);
}
